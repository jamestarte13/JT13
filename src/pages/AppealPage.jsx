...
// ── Upsell screen ─────────────────────────────────────────────────────────────
function UpsellScreen({ email, onContinueFree, onUpgrade }) {
  const [loading, setLoading] = useState(false)
...
function PaymentGate({ email, onSuccess }) {
  const [loading, setLoading] = useState(false)
  ...
  return (
    <div style={{ textAlign: 'center' }}>
      ...
      <div style={{ fontFamily: display, fontSize: 32, color: '#fff', letterSpacing: 2, marginBottom: 8 }}>
        SECOND LETTER
      </div>
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#111', color: '#fff', fontFamily: mono, padding: '32px 20px' }}>
      <div style={{ maxWidth: 560, margin: '0 auto' }}>
        ...
        {screen === 'payment' && (
          <PaymentGate email={email} onSuccess={() => setScreen('result')} />
        )}
